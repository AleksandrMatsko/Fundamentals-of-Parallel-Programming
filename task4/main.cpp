...
void calcNextStage(u_int *stage, u_int *next_stage, int num_lines) {
    for (int i = 0; i < num_lines; i++) {
        for (int j = 0; j < NUM_COLUMNS; j++) {
            ...
            num_alive += getBit(stage[((i + NUM_LINES - 1) % NUM_LINES) * NUM_UINTS + (j + NUM_COLUMNS - 1) % NUM_COLUMNS / UINT_BIT_SIZE],
                                (j + UINT_BIT_SIZE - 1 + (!j) * NUM_COLUMNS) % UINT_BIT_SIZE);
            num_alive += getBit(stage[((i + NUM_LINES - 1) % NUM_LINES) * NUM_UINTS + j / UINT_BIT_SIZE],
                                j % UINT_BIT_SIZE);
            num_alive += getBit(stage[((i + NUM_LINES - 1) % NUM_LINES) * NUM_UINTS + (j + 1) / UINT_BIT_SIZE],
                                (j + 1) * (j != NUM_COLUMNS - 1) % UINT_BIT_SIZE);

            num_alive += getBit(stage[((i + 1) % NUM_LINES) * NUM_UINTS + (j + NUM_COLUMNS - 1) % NUM_COLUMNS / UINT_BIT_SIZE],
                                (j + UINT_BIT_SIZE - 1 + (!j) * NUM_COLUMNS) % UINT_BIT_SIZE);
            num_alive += getBit(stage[((i + 1) % NUM_LINES) * NUM_UINTS + j / UINT_BIT_SIZE],
                                j % UINT_BIT_SIZE);
            num_alive += getBit(stage[((i + 1) % NUM_LINES) * NUM_UINTS + (j + 1) / UINT_BIT_SIZE],
                                (j +  1) * (j != NUM_COLUMNS - 1) % UINT_BIT_SIZE);
